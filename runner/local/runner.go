...
func New(connection interface{}, s *script.Script, arguments interface{}) (*Runner, error) {
	...
	args := strings.SplitN(r.command, " ", 3)
	var cmd *exec.Cmd
	if args[0] == "cmd" {
		...
		fmt.Printf("The args are %v", args)
		cmd = exec.CommandContext(ctx, args[0], args[1:]...)
		// cmd = exec.CommandContext(ctx, "/bin/zsh", "-c", "docker run -ti -v ~/.kube:/root/.kube imb")
	}

	r.cmd = cmd
	// r.cmd.Stdin = stdin
	// stdinTTY := term.NewTTY(stdin)
	// stdinTTY := term.NewTTY(os.Stdin)
	// r.cmd.Stdin = stdinTTY
